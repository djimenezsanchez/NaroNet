...
import NaroNet.Patch_Contrastive_Learning.simclr.resnet as resnet
...
def get_train_steps(num_examples):
  ...
  return (num_examples * FLAGS.PCL_epochs // FLAGS.train_batch_size + 1)
...
def projection_head_Z_dimensions(hiddens, is_training, name='head_contrastive'):
  ...
  with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
    if FLAGS.head_proj_mode == 'none':
      ...
      hiddens = linear_layer(
          hiddens, is_training, FLAGS.Z_dimensions,
      ...
      for j in range(1, FLAGS.num_nlh_layers + 1):
        ...
        hiddens = linear_layer(
            hiddens, is_training, FLAGS.PCL_embedding_dimensions,
